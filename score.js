...
const calculateTotalCheaterScore = (stats) => {
...
players.filter(player => player.competitive_stats?.cheater_score != null);
...
const topPlayers = players.slice(0, 20); // Change the second paramater to get more or less players
...
topPlayers.forEach((player, index) => {
  ...
  console.log(` - Headshot Percentage: Value - ${formatNumber(player.competitive_stats.headshot_percentage * 100, 0)}%, Z-Score - ${formatNumber(player.competitive_stats.z_score_headshotPercentage)}, AVG - ${formatNumber(analyticsData.means.headshotPercentages * 100, 0)}%`);
